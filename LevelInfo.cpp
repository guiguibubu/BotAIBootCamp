...
std::istream& operator>>(std::istream& _is, LevelInfo& _levelInfo)
{
	...
	std::fstream log;
	log.open("D:/inittime.txt", std::ofstream::out);
	...
	while (!ss.eof())
	{
		...
		else if (varName == "MaxNbTurns")
		{
			ss >> _levelInfo.maxTurnNb;
		}
		else if (varName == "NPCCount")
		{
			...
			for (int i = 0; i < nbNPCs; ++i)
			{
				...
				_levelInfo.npcs[npcInfo.npcID] = npcInfo;
			}
			varName = "";
		}
		...
		if (varName == "VisibleObjectCount")
		{
			...
			for (int i = 0; i < nbObjects; ++i)
			{
				...
				_levelInfo.objects[objectInfo.objectID] = objectInfo;
			}
		}
		...
		if (varName == "VisibleTileCount")
		{
			...
			for (int j = 0; j < nbTileCnt; ++j)
			{
				...
				_levelInfo.tiles[tileInfo.tileID] = tileInfo;
			}
		}
	}
