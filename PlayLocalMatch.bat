...
"%AIBOOTCAMP_ENGINE_DIR%/AIBootCamp.exe" -level %mapname% -AIBotPath %aibotpath% -AIBotLogPath %~dp0\LocalMatchResults\aibotlog -GameLogPath %~dp0\LocalMatchResults\gamelog -JobID %JOBNAME% -Team %TEAMNAME% -LoadTime %LoadTime% -InitTime %InitTime% -TurnTime %TurnTime% -quit -batchmode
